...
function renderGradientColorField(name, label, value, escapeHtml) {
  const raw = String(value || "").trim();
  const parsed = colorValue.parseLinearGradient(raw);
  const isGrad = Boolean(parsed);
  const solid = isGrad ? parsed.from : colorValue.normalizeHex(raw, "#6340F5");
  const gradFrom = parsed?.from || solid;
  const gradTo = parsed?.to || colorValue.normalizeHex("#DF0F64");
  const gradDir = parsed?.direction || "135deg";
  const hiddenValue = isGrad ? raw : solid;

  const directionOptions = GRADIENT_DIRECTIONS.map((item) => {
    const selected = gradDir === item.value ? "selected" : "";
    return `<option value="${escapeHtml(item.value)}" ${selected}>${escapeHtml(item.label)}</option>`;
  }).join("");

  const previewBg = isGrad ? raw : solid;

  return `<div class="gc-color-field border border-slate-200 rounded-lg p-3" data-color-field="${escapeHtml(name)}">
    <label class="block text-xs font-semibold text-slate-600 mb-2">${escapeHtml(label)}</label>
    <div class="flex flex-wrap gap-3 text-xs mb-3">
      <label class="inline-flex items-center gap-1.5 cursor-pointer"><input type="radio" name="${escapeHtml(name)}_mode" value="solid" ${isGrad ? "" : "checked"} class="text-indigo-600"> Solid</label>
      <label class="inline-flex items-center gap-1.5 cursor-pointer"><input type="radio" name="${escapeHtml(name)}_mode" value="gradient" ${isGrad ? "checked" : ""} class="text-indigo-600"> Gradient</label>
    </div>
    <div class="gc-solid-panel ${isGrad ? "hidden" : ""} flex items-center gap-2">
      <input type="color" value="${escapeHtml(solid)}" data-solid-picker class="h-10 w-14 border border-slate-300 rounded cursor-pointer">
      <input type="text" data-solid-text value="${escapeHtml(solid)}" class="flex-1 border border-slate-300 rounded-lg px-3 py-2 text-sm font-mono" placeholder="#6340F5">
    </div>
      ...
      <div class="grid grid-cols-2 gap-2">
        <div>
          ...
          <div class="flex items-center gap-2">
            <input type="color" value="${escapeHtml(gradFrom)}" data-gradient-from class="h-10 w-14 border border-slate-300 rounded cursor-pointer">
            <input type="text" value="${escapeHtml(gradFrom)}" data-gradient-from-text class="flex-1 border border-slate-300 rounded-lg px-2 py-1.5 text-xs font-mono">
          </div>
        </div>
        <div>
          <span class="block text-[11px] text-slate-500 mb-1">Color 2</span>
          <div class="flex items-center gap-2">
            <input type="color" value="${escapeHtml(gradTo)}" data-gradient-to class="h-10 w-14 border border-slate-300 rounded cursor-pointer">
            <input type="text" value="${escapeHtml(gradTo)}" data-gradient-to-text class="flex-1 border border-slate-300 rounded-lg px-2 py-1.5 text-xs font-mono">
          </div>
        </div>
      </div>
      <div>
        <span class="block text-[11px] text-slate-500 mb-1">Direction</span>
        <select data-gradient-direction class="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm">${directionOptions}</select>
      </div>
    </div>
    <div class="mt-3 h-9 rounded-lg border border-slate-200" data-color-preview style="background:${escapeHtml(previewBg)}"></div>
    <input type="hidden" name="${escapeHtml(name)}" id="${escapeHtml(name)}" value="${escapeHtml(hiddenValue)}">
  </div>`;
}

function gradientColorFieldScript() {
  return `<script>
(function () {
  ...
  document.querySelector('form[action="/admin/site"], form[action="/admin/site.php"]')?.addEventListener('submit', function () {
    document.querySelectorAll('[data-color-field]').forEach(syncField);
  });
})();
</script>`;
...
module.exports = { renderGradientColorField, gradientColorFieldScript, GRADIENT_DIRECTIONS };
